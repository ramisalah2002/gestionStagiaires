...
function Stagiaire() {
  ...
  const handleDownloadStagiairesPdf = () => {
    ...
    logoImg.onload = () => {
      ...
      const titleX = (doc.internal.pageSize.getWidth() - doc.getStringUnitWidth(titleText) * titleFontSize) +22;
          ...
          <div className="last-stagiaires-content">
            {stagiaires.slice(0, 4).map((stagiaire) => (
              <div key={stagiaire.id} className="last-stagiaire-card">
                ...
                <label className="last-stagiaire-formation">2ème année génie logiciel</label>
                ...
                <div className="download-links-wrapper" ref={containerRef}>
                  ...
                  {showLinks && (
                    <div className="download-links-container">
                      <Link onClick={handleDownloadStagiairesPdf} className="download-link">
                        Télécharger PDF
                      </Link>
                      <Link onClick={handleDownloadStagiairesXlsx} className="download-link">
                        Télécharger XLSX
                      </Link>
                    </div>
                  )}
            ...
            <tbody>
              {stagiaires.map((stagiaire) => (
                <tr key={stagiaire.id}>
                  ...
                  <td>
                    <div className={stagiaire.status === 'Actif' ? 'status-actif' : 'status-inactif'}>
                      {stagiaire.status}
                    </div>
                  </td>
                  <td>
                    <label className="days-in-stage">{stagiaire.joursStage}</label>
                  </td>
